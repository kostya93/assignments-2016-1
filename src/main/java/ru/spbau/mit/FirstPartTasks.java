...
public final class FirstPartTasks {
    ...
    public static long countAlbumDuplicates(Stream<Album> albums) {
        return albums.collect(Collectors.groupingBy(Album::getName, Collectors.counting()))
                .entrySet().stream().filter(stringLongEntry -> stringLongEntry.getValue() > 1).count();
    }
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        return albums.collect(
                Collectors.toMap((Album a) -> a,
                                 (Album a) -> a.getTracks().stream().mapToInt(Track::getRating).max().orElse(0)))
                .entrySet().stream()
    ...
    public static int moduloProduction(IntStream stream, int modulo) {
        return stream.reduce((a1, a2) -> a1 * a2 % modulo).getAsInt();
    }
